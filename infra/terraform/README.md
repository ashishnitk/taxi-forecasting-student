...
`buildspec-dashboard.yml` has no branch gate or lint/test stage: invoking it
builds and pushes the dashboard image and requests an ECS update. Review each
...
cards are not bundled in this repository. The current upload helper and dashboard
buildspec transfer responsible/drift reports and the zone lookup, but not model
cards. A fresh CodeBuild checkout therefore lacks a required Docker `COPY` source.
